...
.EXAMPLE
    Create-Structure -webUrl https://acme.local.com/sites/foo -targetDeployment 004
.EXAMPLE
...
<%
    ...
    {
@'
Import-Module "SharePointPnPPowerShellOnline" -MinimumVersion 3.0.0.0 3>$null
### Authentication
if ($username -and $password) {
    $cred = New-Object System.Management.Automation.PSCredential -argumentlist $username, $password
    Connect-PnPOnline -Url $webUrl -Credentials $cred -SkipTenantAdminCheck
}
else {
    Connect-PnPOnline -Url $webUrl -UseWebLogin -SkipTenantAdminCheck
}
'@
    }
    else 
    {
@'
Import-Module "SharePointPnPPowerShell2013" -MinimumVersion 3.0.0.0 3>$null
### Authentication
...
if ($username -and $password) {
    ...
    Connect-PnPOnline -Url $webUrl -CurrentCredentials -SkipTenantAdminCheck
}
'@
    }
%>
